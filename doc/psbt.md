# PSBT Howto for Odan Core

Since Odan Core 0.17, an RPC interface exists for Partially Signed Odan
Transactions (PSBTs, as specified in
[BIP 174](https://github.com/odan/bips/blob/master/bip-0174.mediawiki)).
...
PSBT is an interchange format for Odan transactions that are not fully signed
...
#### Multisig with multiple Odan Core instances
...
Odan Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `odan-cli` is used.
...
Later, when *V* ODAN has been received on *Amulti*, and Bob and Carol want to
