# User-space, Statically Defined Tracing (USDT) for Odan Core

Odan Core includes statically defined tracepoints to allow for more
...
**Note**: Odan Core uses temporary clones of the _main_ UTXO cache
...
## Adding tracepoints to Odan Core
...
Multiple tools can list the available tracepoints in a `odand` binary with
USDT support.

### GDB - GNU Project Debugger

To list probes in Odan Core, use `info probes` in `gdb`:
...
stap net        inbound_message  0x000000000014419e /src/odand
stap net        outbound_message 0x0000000000107c05 /src/odand
stap validation block_connected  0x00000000002fb10c /src/odand
…
```

### With `readelf`

The `readelf` tool can be used to display the USDT tracepoints in Odan Core.
Look for the notes with the description `NT_STAPSDT`.

```
$ readelf -n ./src/odand | grep NT_STAPSDT -A 4 -B 2
