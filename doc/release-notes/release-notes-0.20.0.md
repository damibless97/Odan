...
Odan Core version 0.20.0 is now available from:

  <https://odancore.org/bin/odan-core-0.20.0/>
...
#### Removal of BIP61 reject network messages from Odan Core

The `-enablebip61` command line option to enable BIP61 has been removed.
(#17004)

This feature has been disabled by default since Odan Core version 0.18.0.
...
- Testing or debugging of implementations of the Odan P2P network protocol
  should be done by inspecting the log messages that are produced by a recent
  version of Odan Core. Odan Core logs debug messages
...
- OpenSSL is no longer used by Odan Core.  (#17265)

- BIP70 support has been fully removed from Odan Core. The
...
- A new `-asmap` configuration option has been added to diversify the
  ...
  single ASN.  See [issue #16599](https://github.com/odan/odan/issues/16599),
  [PR #16702](https://github.com/odan/odan/pull/16702), and the
  `odand help` for more information.  This option is experimental and
...
- All custom settings configured when Odan Core starts are now
...
- The "Start Odan Core on system login" option has been removed on macOS.
  (#17567)
...
- A "known bug" [announced](https://odancore.org/en/releases/0.18.0/#wallet-gui)
  in the release notes of version 0.18 has been fixed.  The issue
  affected anyone who simultaneously used multiple Odan Core wallets
...
- For watch-only wallets, creating a new transaction in the Send screen
  ...
  automatically copy a Partially-Signed Odan Transaction (PSBT) to
  the system clipboard.  This can then be pasted into an external
  program such as [HWI](https://github.com/odan-core/HWI) for
  signing.  Future versions of Odan Core should support a GUI option
...
- Odan Core's automatically-generated source code documentation is
  now available at https://doxygen.odancore.org.  (#17596)
...
- The `odan-cli` utility used with the `-getinfo` parameter now
...
- The `odan-cli` utility now accepts a `-stdinwalletpassphrase`
...
- #17068 Always generate `odanstrings.cpp` on `make translate` (D4nte)
...
- #17741 Included `test_odan-qt` in msvc build (sipsorcery)
...
- #18397 Fix libevent linking for `bench_odan` binary (hebasto)
...
- #18702 Fix ASLR for odan-cli on Windows (fanquake)
...
- #17206 Add testcase to simulate odan schema in leveldb (adamjonas)
...
- #17330 Add `shrinkdebugfile=0` to regtest odan.conf (sdaftuar)
...
- #18022 Fix appveyor `test_odan` build of `*.raw` (MarcoFalke)
...
- #18166 ci: Run fuzz testing test cases (odan-core/qa-assets) under valgrind to catch memory errors (practicalswift)
...
- #18430 ci: Only clone odan-core/qa-assets when fuzzing (MarcoFalke)
...
- #16983 Add detailed info about Odan Core files (hebasto)
...
- #17119 Fix broken odan-cli examples (andrewtoth)
...
- #17503 Remove bitness from odan-qt help message and manpage (laanwj)
- #17539 Update and improve Developer Notes (hebasto)
- #17561 Changed MiniUPnPc link to https in dependencies.md (sandakersmann)
- #17596 Change doxygen URL to doxygen.odancore.org (laanwj)
...
- #17648 Rename wallet-tool references to odan-wallet (hel-o)
...
- #18341 Replace remaining literal ODAN with `CURRENCY_UNIT` (domob1812)
...
- #16115 On odand startup, write config args to debug.log (LarryRuane)
...
- #17650 util: Remove unwanted fields from odan-cli -getinfo (malevolent)
