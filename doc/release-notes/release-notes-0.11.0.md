Odan Core version 0.11.0 is now available from:

  <https://odan.org/bin/odan-core-0.11.0/>
...
defaults to 0.00001. This will cause transactions with fewer ODAN/kB fee to be
...
For example, add the following to `odan.conf`:
...
related to the blockchain in the odan system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows Odan Core to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that Odan Core will 
...
For an example script see [odan-submittx](https://github.com/laanwj/odan-submittx).
...
- #5334 `cf87536` libodanconsensus: Add pkg-config support
...
- #6269 `95aca44` gitian: Use the new odan-detached-sigs git repo for OSX signatures
...
- #5729 `9c4a5a5` Allow unit changes for read-only OdanAmountField
- #5753 `0f44672` Add odan logo to about screen
...
- #5809 `46bfbe7` Add odan-cli man page
...
- #5849 `41259ca` contrib/init/odand.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename Odan-Qt.app to Odan-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for odan-tx)
