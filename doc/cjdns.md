# CJDNS support in Odan Core

It is possible to run Odan Core over CJDNS, an encrypted IPv6 network that
...
done outside of Odan Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the Odan network and individual nodes.
...
before it will work with your Odan Core node. This is described in steps
...
## Run Odan Core with CJDNS

Once you are connected to the CJDNS network, the following Odan Core
...
When enabled, this option tells Odan Core that it is running in an
...
IPv6 local network. This helps Odan Core perform better address management:
...
CJDNS support was added to Odan Core in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `odan-cli -addrinfo` to see the
...
[doc/tor.md](https://github.com/odan/odan/blob/master/doc/tor.md) for
details.

## CJDNS-related information in Odan Core

There are several ways to see your CJDNS address in Odan Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `odan-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `odan-cli getpeerinfo`).

You can use the `getnodeaddresses` RPC to fetch a number of CJDNS peers known to your node; run `odan-cli help getnodeaddresses` for details.
