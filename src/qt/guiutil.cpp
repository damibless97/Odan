...
#include <qt/odanaddressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setValidator(new OdanAddressEntryValidator(parent));
    widget->setCheckValidator(new OdanAddressCheckValidator(parent));
}
...
bool parseOdanURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no odan: URI
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if (!OdanUnits::parse(OdanUnit::ODAN, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
bool parseOdanURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parseOdanURI(uriInstance, out);
}

QString formatOdanURI(const SendCoinsRecipient &info)
{
    ...
    if (info.amount)
    {
        ret += QString("?amount=%1").arg(OdanUnits::format(OdanUnit::ODAN, info.amount, false, OdanUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        ChainType chain = gArgs.GetChainType();
        // Write a odan.desktop file to the autostart directory:
