...
void PaymentServer::ipcParseCommandLine(int argc, char* argv[])
{
    for (int i = 1; i < argc; i++)
    {
        ...
        if (arg.startsWith(BITCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // odan: URI
        {
    ...
    : QObject(parent)
{
    ...
    // on Mac: sent when you click odan: links
...
//
// OSX-specific way of handling odan: URIs
//
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    if (s.startsWith("odan://", Qt::CaseInsensitive))
    {
        Q_EMIT message(tr("URI handling"), tr("'odan://' is not a valid URI. Use 'odan:' instead."),
            CClientUIInterface::MSG_ERROR);
    }
    else if (s.startsWith(BITCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // odan: URI
    {
        QUrlQuery uri((QUrl(s)));
        // normal URI
        {
            SendCoinsRecipient recipient;
            if (GUIUtil::parseOdanURI(s, &recipient))
            {
