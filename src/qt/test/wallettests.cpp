// Copyright (c) 2015-2022 The Odan Core developers
                  ...
                  QMessageBox::StandardButton confirm_type = QMessageBox::Yes)
{
    ...
    entry->findChild<OdanAmountField*>("payAmount")->setValue(amount);
...
void CompareBalance(WalletModel& walletModel, CAmount expected_balance, QLabel* balance_label_to_check)
{
    ...
    QString balanceComparison = OdanUnits::formatWithUnit(unit, expected_balance, false, OdanUnits::SeparatorStyle::ALWAYS);
...
//
//     QT_QPA_PLATFORM=xcb     src/qt/test/test_odan-qt  # Linux
//     QT_QPA_PLATFORM=windows src/qt/test/test_odan-qt  # Windows
//     QT_QPA_PLATFORM=cocoa   src/qt/test/test_odan-qt  # macOS
void TestGUI(interfaces::Node& node, const std::shared_ptr<CWallet>& wallet)
{
    ...
    // Amount input
    OdanAmountField* amountInput = receiveCoinsDialog.findChild<OdanAmountField*>("reqAmount");
