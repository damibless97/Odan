// Copyright (c) 2011-2021 The Odan Core developers
...
void ReceiveRequestDialog::setInfo(const SendCoinsRecipient &_info)
{
    ...
    QString uri = GUIUtil::formatOdanURI(info);
...
{
    if (!model) return;
    ui->amount_content->setText(OdanUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), info.amount));
}

void ReceiveRequestDialog::on_btnCopyURI_clicked()
{
    GUIUtil::setClipboard(GUIUtil::formatOdanURI(info));
}
