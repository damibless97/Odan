...
/** Call node.updateRwSetting() with Odan 22.x workaround. */
static void UpdateRwSetting(interfaces::Node& node, OptionsModel::OptionID option, const std::string& suffix, const common::SettingsValue& value)
{
    if (value.isNum() &&
        ...
        // because Odan 22.x releases try to read these specific settings as
        ...
        // in later releases by https://github.com/odan/odan/pull/24498.
        // If new numeric settings are added, they can be written as numbers
        // instead of strings, because odan 22.x will not try to read these.
...
//! Convert enabled/size values to odan -prune setting.
...
//! Get pruning enabled value to show in GUI from odan -prune setting.
...
//! Get pruning size value to show in GUI from odan -prune setting. If
...
bool OptionsModel::Init(bilingual_str& error)
{
    ...
    // Display
    if (!settings.contains("DisplayOdanUnit")) {
        settings.setValue("DisplayOdanUnit", QVariant::fromValue(OdanUnit::ODAN));
    }
    QVariant unit = settings.value("DisplayOdanUnit");
    if (unit.canConvert<OdanUnit>()) {
        m_display_odan_unit = unit.value<OdanUnit>();
    } else {
        m_display_odan_unit = OdanUnit::ODAN;
        settings.setValue("DisplayOdanUnit", QVariant::fromValue(m_display_odan_unit));
    }
...
void OptionsModel::SetPruneTargetGB(int prune_target_gb)
{
    ...
    // from saved value. Avoid writing settings.json if odan.conf value
...
#endif
    case DisplayUnit:
        return QVariant::fromValue(m_display_odan_unit);
...
{
    if (new_unit.isNull() || new_unit.value<OdanUnit>() == m_display_odan_unit) return;
    m_display_odan_unit = new_unit.value<OdanUnit>();
    QSettings settings;
    settings.setValue("DisplayOdanUnit", QVariant::fromValue(m_display_odan_unit));
    Q_EMIT displayUnitChanged(m_display_odan_unit);
}
...
void OptionsModel::checkAndMigrate()
{
    ...
    if (settingsVersion < CLIENT_VERSION)
    {
        ...
        // see https://github.com/odan/odan/pull/8273
    ...
    // (https://github.com/odan-core/gui/issues/567).
