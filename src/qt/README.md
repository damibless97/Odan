This directory contains the source code for the Odan Core graphical user interface (GUI). It uses the [Qt](https://www1.qt.io/developers/) cross-platform framework.
...
- Represents the main window of the Odan UI.
...
- ClientModel is used by the main application `odangui` and several models like `peertablemodel`.
...
- (Deprecated) Used to process BIP21 payment URI requests. Also handles URI-based application switching (e.g. when following a odan:... link from a browser).
...
* UI elements like OdanAmountField, which inherit from QWidget.
* `odanstrings.cpp`: automatically generated
* `odanunits.(h/cpp)`: ODAN / mODAN / etc. handling
* `callback.h`
...
**Note:** Do not change `local/odan_en.ts`. It is updated [automatically](/doc/translation_process.md#writing-code-with-translations).
...
4. Enter `odan-qt` as the Project Name and enter the absolute path to `src/qt` as Location
...
8. While in the `Projects` tab, ensure that you have the `odan-qt` executable specified under `Run`
 - If the executable is not specified: click `"Choose..."`, navigate to `src/qt`, and select `odan-qt`
9. You're all set! Start developing, building, and debugging the Odan Core GUI
